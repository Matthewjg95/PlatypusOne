...
| `LinkedDisplay` + `SerialTransport` + `--board unoq` wiring | Presentation-link host side | ✅ **Verified 2026-09-02** host-side: 14 suites incl. the scripted loopback client (session, tiling, ack timeout/drop, input, backlight); physical session per status board steps 5–6 |
| Linux/GCC CI job | Whole tree on the target's OS/compiler family | ✅ **GREEN 2026-09-02** after four fix rounds: two GCC nested-aggregate default-argument rejections (UnoQBoard, FakeCamera) and a latent PR #11 include-path bug that had made the capture tool uncompilable on Linux. V4l2Camera, UnoQBoard, SerialMcuBridge, SerialTransport now compile and all 14 suites pass on ubuntu-latest |
